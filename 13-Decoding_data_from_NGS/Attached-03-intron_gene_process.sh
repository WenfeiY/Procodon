...
#  Input NGS data path
FQ1_path=$in_fq1_path
FQ2_path=$in_fq2_path
...
#  BLASTN results of reads aligned to intron sequences
BLASTN_intron_1_out=$BLASTN_dir/$gene.intron.blastn.1.out
BLASTN_intron_2_out=$BLASTN_dir/$gene.intron.blastn.2.out

#  TBLASTN (reads to protein sequences, pair-ended)
...
#  BLASTN align introns and reads
...
cut -f2 $fil_TBLASTN1_out | sort -u > $Read_list && \
    ...
    cut -f2 $BLASTN_intron_1_out | sort -u >> $Read_list && \
    cut -f2 $BLASTN_intron_2_out | sort -u >> $Read_list && \
...
spades.py -o $spades_dir/ -1 $FQ1_sub -2 $FQ2_sub --careful -k 21,33,55,77,99 -t 8 
